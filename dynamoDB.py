...
class DynamoDB:
    ...
    def getTweetsForView(self):
        ...
        result = self.tweetsTable.scan(time__beginswith=utc_time)#,limit=5000)
        #timenow = time.time()
        for x in result:
    ...
    def getTweetsBySearch(self,search):
        ...
        result1 = self.tweetsTable.scan(time__beginswith=utc_time)
        result2 = self.tweetsTable.scan(time__beginswith=utc_time_y)
